...
REAL*8, PARAMETER :: m=100000.0d0, k=1.d0, kb=3.16e-6                                         ! au
...
OPEN(30,FILE="force_file",STATUS="REPLACE")
...
print *, 'deltaT in kelvin =', deltaT
deltaT=deltaT*kb
print *, 'deltaT in atomic units =',deltaT
mtd_step=0

ALLOCATE(xtau(nsteps/100))
ALLOCATE(ytau(nsteps/100))
ALLOCATE(hx(nsteps/100))
ALLOCATE(hy(nsteps/100))
hx=0.d0
hy=0.d0

PRINT*,'height of gaussian hx=',hx0,'and hy=', hy0, 'width of gaussian x=',dsx, 'gaussian y=', dsy
  
  DO i=1,natoms
  READ(1,*) x,y
  END DO

print *, 'system temperature in kelvin =', system_t
au_t = system_t*kb
print *, 'system temperature in atomic units =', au_t

!...setting initial velocities
  CALL temp(bath_temp,kb,au_tbath)
  CALL rand_vel(vel,natoms)
vel_x = vel*sqrt(m/(2.d0*3.14*au_tbath))
PRINT *, "vel_x", vel_x  
CALL rand_vel(vel,natoms)
...
print *, 'temperature of randomly generated velocity in kelvin=', t
 vel_x=sqrt(bath_temp/t)*vel_x
 vel_y=sqrt(bath_temp/t)*vel_y
print *, 'velocity after scaling vx=', vel_x, 'and vy=', vel_y
!.....
PRINT *, 'bath T in kelvin=', bath_temp
print *, 'bath temperature in atomic units =', au_tbath
  
  CALL fion_x(x,y,force_x,m,a,k,kb,natoms,f,au_tbath,dt,noise,mtd_step,xtau,hx,dsx)
  CALL fion_y(x,y,force_y,m,b,k,kb,natoms,f,au_tbath,dt,noise,mtd_step,ytau,hy,dsy)
...
mtd_time_step=100
  ...
  CALL new_vel_x(vel_x,force_x,dt,natoms,f)   
  CALL new_vel_y(vel_y,force_y,dt,natoms,f)   
 ...
 displaced = .false.
     ...
     mtd_step=mtd_step+1
         ...
         call eval_vbias(y,ytau,mtd_step,dsy,hy,vbias)
          hy(mtd_step)=hy0*dexp(-vbias/deltaT)             ! deltaT is in atomic units.
...
SUBROUTINE fion_x(x,y,force_x,m,a,k,kb,natoms,f,au_tbath,dt,noise,mtd_step,xtau,h,ds)
implicit none
REAL*8 :: x,y,force_x,xtau(*),h(*)
...
noise = dsqrt(2*f*au_tbath/dt)*2.0d0*(d1-0.5)                   ! f units= m/dt,   kb is not multiplied because the temperature is already in atomic units.
    DO i=1,natoms
...
SUBROUTINE fion_y(x,y,force_y,m,b,k,kb,natoms,f,au_tbath,dt,noise,mtd_step,ytau,h,ds)
implicit none
REAL*8 :: x,y,force_y,ytau(*),h(*)
...
noise = dsqrt(2*f*au_tbath/dt)*2.0d0*(d1-0.5)                              ! kb is not multiplied because the temperature is already in atomic units
    DO i=1,natoms
    force_y = -v0*(4.0d0*y/(m*b**4))*(y**2-b**2) + (1.0/m)*noise        ! the stochastic term is not coming from surface potential. 
         DO l=1,mtd_step-1
...
SUBROUTINE new_vel_x(vel_x,force_x,dt,natoms,f)   
implicit none
REAL*8 :: vel_x,force_x
REAL*8 :: dt,f
INTEGER :: natoms,i
   DO i = 1,natoms
    vel_x =  vel_x*(1-0.5*f*dt/m) + 0.5*dt*force_x                       ! this velocity friction is due to stochastic force.
  END DO
END SUBROUTINE
   
SUBROUTINE new_vel_y(vel_y,force_y,dt,natoms,f)   
implicit none
REAL*8 :: vel_y,force_y
REAL*8 :: dt,f
INTEGER :: natoms,i
   DO i = 1,natoms
    vel_y =  vel_y*(1-0.5*f*dt/m) + 0.5*dt*force_y                       ! this velocity friction is due to stochastic force.
  END DO
END SUBROUTINE
...
REAL*8 :: pet,a,k,bias,dsx,dsy,b,v0
INTEGER :: natoms,i,l,mtd_step,p
bias = 0.0d0
v0=0.001d0
      DO p=1,natoms
      pet=  v0*((x**2 - a**2)/(a**2))**2+v0*((y**2-b**2)/b**2)**2
